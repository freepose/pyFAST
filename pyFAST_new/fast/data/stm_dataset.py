...
"""
    Single prediction Target Multiple sources time series dataset.
...
class STMDataset(data.Dataset):
    """
        Single prediction Target Multiple sources (STM) time series dataset.

        ``STMDataset`` transforms **several** time series to (masked) input / output data.
    ...
    def index_dataset(self, ts: tuple[torch.Tensor] or list[torch.Tensor],
                      ts_mask: tuple[torch.Tensor] or list[torch.Tensor] = None,
                      ex_ts: tuple[torch.Tensor] or list[torch.Tensor] = None,
                      ex_ts_mask: tuple[torch.Tensor] or list[torch.Tensor] = None, ) -> np.array:
        """
