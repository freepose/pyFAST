...
from example.prepare_sh_diabetes import load_sh_diabetes_stm as load_diabetes_stm
from example.prepare_industrial_power_load import load_industrial_power_load_stm
...
def ms_ts_fusion():
    ...
    (train_ds, val_ds), (scaler, ex_scaler) = load_diabetes_stm(data_root, 'all', ds_params, 'inter', 0.8)

    # ds_params = {'input_window_size': 8 * 24, 'output_window_size': 24, 'horizon': 1, 'split_ratio': 0.8}
    # (train_ds, val_ds), (scaler, ex_scaler) = \
    #     load_industrial_power_load_stm(data_root, 'mice', None, ['temperature', 'humidity'], ds_params)
    ...
    model_cls, user_settings = modeler['sparse-narx-rnn']
