...
"""
    Prepare datasets by loading greek-wind-energy-forecasting as ``SSTDataset``.
"""
...
def load_greece_wpf_sst(data_root: str,
                        use_ex_variables: bool = True,
                        use_time_variable: bool = False,
                        time_granularity: Literal['1day', '12hour', '6hour', '1hour'] = '1day',
    ...
    """
        Load Greek wind power dataset as ``SSTDataset``.
    ...
    csv_dir = os.path.expanduser(data_root) + r'time_series/energy_wind/greek-wind-energy-forecasting/2_mts_csv'
    csv_file = csv_dir + f'/greek_wpf_{time_granularity}.csv'

    time_variable = 'Datetime' if use_time_variable else None
    ...
    target_df = df.iloc[:, [i for i, x in enumerate(df.columns) if "power" in x]]
    ...
    if use_ex_variables:
        ex_df = df.iloc[:, [i for i, x in enumerate(df.columns) if "power" not in x and "Datetime" not in x]]
