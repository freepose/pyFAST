...
class Triformer(nn.Module):
    """
        ...
        :param channels: number of hidden channels (default: 32), controls the model's representation capacity.
        :param patch_sizes: subject to patch sizes can be divided by input_window_size.
        :param mem_dim: memory state dimension (default: 5), affects long-term dependency modeling
                (effective only when factorized=True).
    """

    def __init__(self, input_window_size: int = 1, input_vars: int = 1, output_window_size: int = 1,
                 channels: int = 32, patch_sizes: List[int] = [5, 2], mem_dim: int = 5):
        ...
        self.lag = self.input_window_size
        ...
        for patch_size in patch_sizes:
            ...
            self.skip_generators.append(WeightGenerator(in_dim=cuts * channels, out_dim=256, number_of_weights=1,
                                                        mem_dim=mem_dim, num_nodes=self.num_nodes, factorized=False))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_enc = x
        prediction = self.forward_xformer(x_enc)
        prediction = prediction.squeeze(-1)
        return prediction

    def forward_xformer(self, batch_x):
        ...
        out = self.projections(x).transpose(2, 1).unsqueeze(-1)
        return out
