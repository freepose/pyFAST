...
class FEDformerMaskEx2(nn.Module):
    """
    ...
    def __init__(self, input_window_size: int = 1, input_vars: int = 1,
                 ...
                 modes: int = 32):
        super(FEDformerMaskEx2, self).__init__()
        assert output_vars == input_vars, 'Invalid window parameters.'
        assert input_window_size > 0, 'Invalid input_window_size parameters.'
        assert output_window_size > 0, 'Invalid output_window_size parameters.'
    ...
    def forward(self, x: torch.Tensor, x_mask: torch.Tensor = None, ex2: torch.Tensor = None) -> torch.Tensor:
        ...
        seasonal_init, trend_init = self.decompose_series(xe)
        mean = torch.mean(xe, dim=1).unsqueeze(1).repeat(1, self.output_window_size, 1) # -> (batch_size, input_window_size, input_vars)
