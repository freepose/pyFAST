...
class AutoformerMaskEx2(nn.Module):
    """
    ...
    def __init__(self, input_vars: int = 1, output_window_size: int = 1, output_vars: int = 1,
        ...
        super(AutoformerMaskEx2, self).__init__()
    ...
    def forward(self,x: torch.Tensor, x_mask: torch.Tensor = None, ex2: torch.Tensor = None) -> torch.Tensor:
        ...
        # encoder_embedding
        x_embedding = self.encoder_value_embedding(xe) # -> [batch_size, input_window_size, d_model]
        ...
        mean = torch.mean(xe, dim=1).unsqueeze(1).repeat(1, self.output_window_size, 1)
