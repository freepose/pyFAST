...
class FEDformerEx2(nn.Module):
    ...
    def __init__(self, input_window_size: int = 1, input_vars: int = 1,
                 ...
                 modes: int = 32):
        super(FEDformerEx2, self).__init__()
        ...
        if self.temporal_vars:
            self.encoder_temporal_embedding = nn.Linear(self.temporal_vars, self.d_model, bias=False)
        ...
        if self.temporal_vars:
            self.decoder_temporal_embedding = nn.Linear(self.temporal_vars, self.d_model, bias=False)
    ...
    def forward(self, x: torch.Tensor, ex2: torch.Tensor = None) -> torch.Tensor:
        """
        ...
        if self.temporal_vars and ex2 is not None:
            x_embedding += self.encoder_temporal_embedding(ex2_inputs)
        x_embedding = self.encoder_dropout(x_embedding)

        # -> (batch_size, label_window_size + input_window_size, d_model)
        target_embedding = self.decoder_value_embedding(seasonal_init)+ self.decoder_position_embedding(seasonal_init)
        if self.temporal_vars and ex2 is not None:
